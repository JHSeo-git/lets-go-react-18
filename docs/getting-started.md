...
webpack + babel로 번들링을 구성하고 진행해보았습니다.
...
`createRoot()`는 전달된 container node의 content를 제어합니다. 내부의 기존 DOM element는 render가 호출될 때 교체됩니다. 이후 호출은 효율적인 업데이를 위해 React의 DOM diffing 알고리즘을 사용합니다.
